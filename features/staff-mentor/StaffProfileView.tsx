...
const StaffProfileView: React.FC<{ staffId: string }> = ({ staffId }) => {
  ...
  const handleExportPPTX = () => {
    ...
    slides.forEach((slide: any) => {
      ...
      s.background = { color: '0F172A' }; // Slate 950

      // Title
      s.addText(slide.title, {
        x: 0.5, y: 0.5, w: '90%', h: 1,
        fontSize: 32, bold: true, color: 'EA580C', transform: { uppercase: true }
      });
    ...
    <div className="space-y-8 animate-scale-in">
      ...
      <div className="bg-white p-12 rounded-[4.5rem] border border-slate-100 shadow-2xl relative overflow-hidden group">
         <div className="flex flex-col xl:flex-row justify-between items-center gap-10 relative z-10">
            ...
            <div className="flex gap-4">
               ...
               >
                  {isAiProcessing ? 'NÖRAL ANALİZ AKTİF...' : (idp ? 'GELİŞİMİ YENİLE' : 'PLANI BAŞLAT')}
               </button>
               {idp?.autoGeneratedPresentation && (
                 <div className="flex bg-slate-100 p-1 rounded-[2.5rem]">
                    <button onClick={() => { setActiveSlide(0); setIsFullscreen(true); }} className="px-8 py-5 bg-white text-slate-900 rounded-[2.2rem] text-[11px] font-black uppercase tracking-widest shadow-sm hover:bg-orange-600 hover:text-white transition-all">SUNUMU BAŞLAT</button>
                    <button onClick={handleExportPPTX} className="px-6 py-5 text-slate-400 hover:text-orange-600 transition-all">
         ...
         <div className="absolute -right-40 -top-40 w-80 h-80 bg-orange-600/5 rounded-full blur-[120px]"></div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
         ...
         <div className="lg:col-span-4 bg-slate-900 p-12 rounded-[5rem] text-white shadow-2xl flex flex-col">
            <h4 className="text-[11px] font-black text-orange-500 uppercase tracking-[0.5em] mb-10 border-b border-white/10 pb-6">Aktif Yetkinlikler</h4>
            <div className="flex-1 space-y-3 overflow-y-auto custom-scrollbar pr-2">
               {(data.profile.all_trainings || []).map((t: string, i: number) => (
                  <div key={i} className="p-4 bg-white/5 rounded-2xl border border-white/5 flex items-center gap-4 group hover:bg-white/10 transition-colors">
                     ...
                     <span className="text-[11px] font-black uppercase tracking-tight text-slate-300">{t}</span>
                  </div>
               ))}
            </div>
         </div>
      ...
      {idp ? (
           ...
           <div className="lg:col-span-5 space-y-8">
              <div className="p-12 bg-white rounded-[4rem] border-2 border-orange-500/20 shadow-2xl relative overflow-hidden">
                 <div className="flex items-center gap-4 mb-10">
                    ...
                    <h5 className="text-[14px] font-black text-slate-900 uppercase tracking-[0.4em]">Odak: {idp.focusArea}</h5>
                 </div>
              ...
              <div className="p-12 bg-slate-950 rounded-[4rem] text-white shadow-3xl">
                 ...
                 <div className="space-y-12">
                    <div className="border-l-2 border-white/10 pl-10 relative">
                       <span className="text-[10px] font-black text-slate-500 uppercase block mb-4">3 AY: STABİLİZASYON</span>
                       <p className="text-[17px] font-bold text-slate-200 italic leading-relaxed uppercase">"{idp.roadmap.shortTerm}"</p>
                    </div>
                    <div className="border-l-2 border-white/10 pl-10 relative">
                       <span className="text-[10px] font-black text-slate-500 uppercase block mb-4">12 AY: UZMANLIK</span>
                       <p className="text-[17px] font-bold text-slate-200 italic leading-relaxed uppercase">"{idp.roadmap.longTerm}"</p>
                    </div>
                 </div>
              </div>
           </div>
           ...
           <div className="lg:col-span-7">
              {idp.autoGeneratedPresentation && idp.autoGeneratedPresentation.length > 0 ? (
                ...
                <div className="h-full flex flex-col items-center justify-center border-4 border-dashed border-slate-100 rounded-[5rem] opacity-30 text-center p-20">
                   <h3 className="text-3xl font-black text-slate-400 uppercase tracking-[0.8em]">Müfredat Hazırlanıyor...</h3>
                </div>
              )}
           </div>
        </div>
        ...
        <div className="py-32 bg-slate-50 rounded-[5rem] border-4 border-dashed border-slate-200 text-center flex flex-col items-center justify-center">
           <button onClick={handleGenerateIDP} className="px-12 py-6 bg-slate-900 text-white rounded-3xl font-black uppercase text-[11px] tracking-widest hover:bg-orange-600 transition-all shadow-xl">GELİŞİM ANALİZİNİ TETİKLE</button>
        </div>
      )}
    </div>
  );
    ...
    <div className={`flex flex-col justify-between h-full transition-all duration-700 ${full ? 'p-16 md:p-32 bg-slate-950 text-white' : 'p-12 bg-white'}`}>
      <div className="space-y-12">
        <div className="flex justify-between items-center relative z-[1001]">
           ...
           <div className="flex gap-4">
              {full && <button onClick={onClose} className="p-4 bg-white/5 hover:bg-rose-600 rounded-full transition-all text-white"><svg className="w-10 h-10" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={3}><path d="M6 18L18 6M6 6l12 12" /></svg></button>}
           </div>
        </div>
        ...
        <div className="space-y-8 relative z-10">
          ...
          {slide.subtitle && (
            <p className={`font-bold italic leading-relaxed opacity-90 ${full ? 'text-4xl md:text-5xl text-slate-400 max-w-6xl' : 'text-xl text-slate-500'}`}>"{slide.subtitle}"</p>
          )}
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-12 mt-16 relative z-10">
           <div className="space-y-8">
              <span className={`text-[10px] font-black uppercase tracking-widest block border-b pb-4 ${full ? 'text-slate-500 border-white/10' : 'text-slate-400 border-slate-100'}`}>UYGULAMA STRATEJİLERİ</span>
              <div className="space-y-4">
           ...
           {slide.visualPrompt && (
             <div className={`p-12 rounded-[4rem] border flex flex-col justify-center relative overflow-hidden ${full ? 'bg-orange-600/10 border-orange-500/20' : 'bg-orange-50 border-orange-100'}`}>
                <span className="text-[10px] font-black text-orange-500 uppercase tracking-widest block mb-6">KLİNİK PROJEKSİYON</span>
                <p className={`font-bold italic leading-relaxed ${full ? 'text-3xl text-orange-100' : 'text-base text-orange-950'}`}>{slide.visualPrompt}</p>
             </div>
           )}
        </div>
      </div>

      <div className={`mt-20 flex justify-between items-center pt-10 border-t ${full ? 'border-white/10' : 'border-slate-100'} relative z-[1001]`}>
        <div className="flex gap-6">
           <button disabled={index === 0} onClick={onPrev} className={`p-8 rounded-full transition-all border-4 ${full ? 'bg-white/5 border-white/10 text-white hover:bg-orange-600' : 'bg-slate-900 border-slate-900 text-white hover:bg-orange-600'} disabled:opacity-20`}>
              <svg className="w-10 h-10" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={4}><path d="M15 19l-7-7 7-7" /></svg>
           </button>
           <button disabled={index === total - 1} onClick={onNext} className={`p-8 rounded-full transition-all border-4 ${full ? 'bg-white/5 border-white/10 text-white hover:bg-orange-600' : 'bg-slate-900 border-slate-900 text-white hover:bg-orange-600'} disabled:opacity-20`}>
        ...
        {slide.interactiveElement && (
          <div className="text-right max-w-xl">
             <p className="text-[11px] font-black text-slate-500 uppercase tracking-[0.4em] mb-3">SORU & TARTIŞMA</p>
             <p className={`font-black uppercase italic tracking-tighter ${full ? 'text-3xl text-orange-500' : 'text-xl text-orange-600'}`}>"{slide.interactiveElement.question}"</p>
          </div>
        )}
      </div>
    </div>
