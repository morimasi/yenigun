...
const CandidateDetail: React.FC<{ candidate: Candidate, config: GlobalConfig, onUpdate: (c: Candidate) => void, onDelete: () => void }> = ({ candidate, config, onUpdate, onDelete }) => {
  ...
  const handlePermanentDelete = async () => {
    if (isDeleteConfirmOpen) {
       onDelete(); // Parent (PipelineView) handles the service call and state update
    } else {
       setIsDeleteConfirmOpen(true);
       setTimeout(() => setIsDeleteConfirmOpen(false), 5000); // 5 sec auto-reset
    }
  };
  ...
  const renderMatrix = () => {
    ...
    if (!data) return <div className="p-10 text-center opacity-30 uppercase font-black tracking-widest">Analiz Verisi Bekleniyor</div>;
        ...
        <div className="md:col-span-4 flex flex-col gap-2 overflow-y-auto custom-scrollbar pr-2 h-[600px]">
           {['KLİNİK', 'ETİK', 'KURUMSAL'].map(groupName => (
             <div key={groupName} className="space-y-1 mb-4">
                <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-4 mb-2 block">{groupName} ANALİZİ</span>
        ...
        <div className="md:col-span-8 space-y-6 overflow-y-auto custom-scrollbar h-[600px] pr-2">
           <div className="bg-white p-10 rounded-[3.5rem] border border-slate-200 shadow-sm relative overflow-hidden group">
              ...
              <div className="flex justify-between items-start mb-8">
                 <div>
                    <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.4em] mb-2">Klinik Muhakeme Gerekçesi</h4>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                 <div className="space-y-6">
                    <div className="bg-slate-50 p-6 rounded-3xl border border-slate-100">
                        ...
                        <p className="text-[11px] font-medium text-slate-600 leading-relaxed italic">"{data.clinicalNuances || 'Bu kategori için spesifik nüans saptanmadı.'}"</p>
                    </div>
                    <div className="bg-blue-50/50 p-6 rounded-3xl border border-blue-100">
                        <span className="text-[9px] font-black text-blue-700 uppercase tracking-widest block mb-3">BİLİMSEL LİTERATÜR ATIFLARI</span>
                        <p className="text-[11px] font-bold text-blue-900 leading-relaxed">"{data.literatureReference || 'Genel klinik prensiplerle uyumlu.'}"</p>
                    </div>
                 </div>

                 <div className="space-y-6">
                    <div className="bg-slate-900 p-6 rounded-3xl text-white shadow-xl">
                        <span className="text-[9px] font-black text-orange-500 uppercase tracking-widest block mb-3">EKİP ÜZERİNDEKİ POTANSİYEL ETKİ</span>
                        <p className="text-[11px] font-bold text-slate-300 leading-relaxed italic">"{data.teamImpact || 'Nötr etki öngörülüyor.'}"</p>
                    </div>
                    <div className="grid grid-cols-1 gap-3">
                       <div className="bg-emerald-50/50 p-5 rounded-2xl border border-emerald-100">
                          <span className="text-[8px] font-black text-emerald-700 uppercase block mb-2">GÜÇLÜ YÖNLER</span>
  ...
  const renderDNA = () => (
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 animate-scale-in">
       <div className="lg:col-span-5 bg-white p-8 rounded-[3.5rem] border border-slate-200 shadow-sm flex flex-col items-center">
          <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em] mb-8 w-full text-left">Yetkinlik Spektrumu</h4>
       ...
       <div className="lg:col-span-7 space-y-6">
          ...
          <div className="bg-white p-8 rounded-[3rem] border border-slate-200 shadow-sm">
             <h4 className="text-[10px] font-black text-slate-900 uppercase tracking-[0.4em] mb-4 border-l-4 border-orange-600 pl-6">Klinik Karakter Analizi</h4>
             <p className="text-[13px] font-medium text-slate-600 leading-relaxed text-justify italic">
                "{candidate.report?.detailedAnalysisNarrative || "Analiz süreci bekleniyor..."}"
             </p>
          </div>
       </div>
    </div>
            ...
            <div className="h-[300px] w-full">
               <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={trajectory}>
                     ...
                     <Area type="monotone" dataKey="burnoutRisk" stroke="#ef4444" strokeWidth={2} strokeDasharray="5 5" fill="transparent" name="Tükenmişlik %" />
                  </AreaChart>
               </ResponsiveContainer>
            </div>
         </div>
    ...
    <div className="space-y-6 animate-fade-in">
       <div className="bg-white p-8 rounded-[3.5rem] border border-slate-200 shadow-md">
          <div className="flex items-center gap-4 mb-10 border-l-8 border-slate-900 pl-8">
             ...
             <span className="px-4 py-1.5 bg-orange-600 text-white rounded-xl text-[10px] font-black uppercase tracking-widest">STRATEJİK</span>
          </div>
                ...
                <div key={i} className="flex gap-6 items-start group p-6 hover:bg-slate-50 rounded-[2.5rem] transition-all border border-transparent hover:border-slate-100">
                   ...
                   <div className="space-y-2">
                      ...
                      <p className="text-[10px] font-black text-orange-600 uppercase tracking-widest opacity-0 group-hover:opacity-100 transition-all">
                        * Adayın metodolojik dürüstlüğünü test eder.
                      </p>
                   </div>
                </div>
             ))}
      ...
      <div className="h-16 border-b border-slate-200 flex items-center justify-between px-6 bg-white shrink-0 shadow-sm z-10">
         ...
         <div className="flex items-center gap-3">
            ...
            <button 
              ...
              className={`px-5 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${isDeleteConfirmOpen ? 'bg-rose-600 text-white animate-pulse' : 'bg-rose-50 text-rose-600 hover:bg-rose-100'}`}
            >
