  ...
  const OptionEditor = () => {
    ...
    const updateOption = (idx: number, field: string, value: any) => {
      if (targetType === 'CANDIDATE') {
        const next = [...editingItem.weightedOptions];
        if (field === 'weights') {
           // Weight parsing logic (simple: clinical:0.5)
           // For simplicity in this UI, we assume editing a specific weight key like 'clinical'
           // This is a simplified handler. Real implementation would need a nested object editor.
           // Here we will just provide inputs for specific keys in the UI render part.
        } else {
