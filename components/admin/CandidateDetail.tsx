...
const AnalysisPoint: React.FC<{ title: string; data: any; color: string }> = ({ title, data, color }) => (
  <div className="flex flex-col gap-3 py-5 border-b border-slate-50 last:border-0 group hover:bg-slate-50/50 px-4 -mx-4 rounded-xl transition-all">
    ...
    {/* Yeni Derin Analiz Alanları */}
    <div className="grid grid-cols-2 gap-4 mt-2">
      <div className="p-3 bg-white/50 rounded-xl border border-slate-100 shadow-sm">
        <span className="text-[7px] font-black text-slate-400 uppercase block mb-1">Kısa Vadeli Etki (0-3 Ay)</span>
        <p className="text-[9px] font-semibold text-slate-600 leading-snug">{data.shortTermImpact || 'Analiz ediliyor...'}</p>
      </div>
      <div className="p-3 bg-white/50 rounded-xl border border-slate-100 shadow-sm">
        <span className="text-[7px] font-black text-slate-400 uppercase block mb-1">Uzun Vadeli Projeksiyon (1 Yıl)</span>
        <p className="text-[9px] font-semibold text-slate-600 leading-snug">{data.longTermImplication || 'Analiz ediliyor...'}</p>
      </div>
    </div>
  </div>
);
...
const CandidateDetail: React.FC<{ candidate: Candidate, config: GlobalConfig, onUpdate: (c: Candidate) => void, onDelete: () => void }> = ({ candidate, config, onUpdate, onDelete }) => {
  ...
  const handleRunAnalysis = async () => {
    ...
    try {
      ...
      if (!aiReport) throw new Error("Yapay zeka motorundan boş yanıt döndü.");
  ...
  return (
    <div className="bg-white rounded-[2rem] shadow-2xl border border-slate-100 h-full flex flex-col overflow-hidden animate-scale-in">
      ...
      <div className="p-6 border-b border-slate-100 bg-white flex justify-between items-center no-print">
        ...
        >
          {isAnalysing ? 'SİSTEM ANALİZ EDİYOR...' : 'ANALİZİ GÜNCELLE'}
        </button>
      </div>

      <div className="flex-1 overflow-y-auto p-6 space-y-6 custom-scrollbar bg-[#F8FAFC]">
        {successStatus && (
          <div className="p-3 bg-emerald-600 text-white rounded-xl flex items-center gap-2 animate-slide-up font-black text-[8px] uppercase tracking-widest shadow-lg">
          ...
          <div className="grid grid-cols-1 gap-6">
            ...
            <div className="grid grid-cols-1 gap-6">
              <div className="bg-white rounded-[1.5rem] border border-slate-100 shadow-sm p-6 space-y-2">
                <h4 className="text-[9px] font-black text-slate-900 uppercase tracking-widest mb-4 flex items-center gap-2">
                   <span className="w-1.5 h-1.5 bg-blue-600 rounded-full"></span> II. Teknik ve Klinik Kriterler
                </h4>
            ...
            <div className="bg-slate-900 rounded-[1.5rem] p-8 text-white relative overflow-hidden group">
               <div className="relative z-10">
                  <h4 className="text-[9px] font-black text-orange-500 uppercase tracking-[0.4em] mb-3">III. Nihai Karar Tavsiyesi</h4>
