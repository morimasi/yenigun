...
import { FORM_STEPS, BRANCH_QUESTIONS, CERTIFICATION_LIST, TURKISH_UNIVERSITIES, TURKISH_DEPARTMENTS } from '../constants';
...
const CandidateForm: React.FC<CandidateFormProps> = ({ onSubmit }) => {
  ...
  // BRANŞ BAZLI DİNAMİK SORU FİLTRELEME
  const currentQuestions = useMemo(() => {
    const stepId = FORM_STEPS[currentStep].id;
    const allQuestionsInStep = BRANCH_QUESTIONS[stepId] || [];
    
    // Branş seçildiyse, o branşa uygun olmayan soruları filtrele
  ...
  const handleNext = () => {
    ...
    if (stepId === 'personal') {
      ...
      if (unanswered.length > 0) {
        alert("Lütfen bu bölümdeki tüm klinik vakaları değerlendirerek ilerleyiniz.");
        return;
      }
    }
      ...
      return (
        <div className="space-y-6 animate-fade-in">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-5">
            <div className="lg:col-span-8 bg-white p-5 md:p-8 rounded-[2rem] border border-slate-100 shadow-sm space-y-4">
               ...
               <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div className="space-y-1.5">
                    ...
                    <input type="email" className="w-full rounded-xl border border-slate-100 p-3 font-bold text-slate-800 outline-none bg-slate-50/30" value={formData.email} onChange={(e) => setFormData({...formData, email: e.target.value})} placeholder="E-POSTA" />
                  </div>
                  <div className="space-y-1.5">
                    <label className="text-[9px] font-black text-slate-400 uppercase ml-1">Telefon</label>
                    <input type="text" className="w-full rounded-xl border border-slate-100 p-3 font-bold text-slate-800 outline-none bg-slate-50/30" value={formData.phone} onChange={(e) => setFormData({...formData, phone: e.target.value})} placeholder="TELEFON" />
                  </div>
                  <div className="space-y-1.5">
                    <label className="text-[9px] font-black text-slate-400 uppercase ml-1">Yaş / Cinsiyet</label>
                    <div className="flex gap-2">
                       <input type="number" className="w-16 rounded-xl border border-slate-100 p-3 font-bold text-slate-900 outline-none bg-slate-50/30 text-center" value={formData.age} onChange={(e) => setFormData({...formData, age: parseInt(e.target.value) || 0})} />
                       <select className="flex-1 rounded-xl border border-slate-100 p-3 font-bold text-slate-900 outline-none bg-slate-50/30" value={formData.gender} onChange={(e) => setFormData({...formData, gender: e.target.value as Gender})}>
            ...
            <div onClick={() => fileInputRef.current?.click()} className={`lg:col-span-4 p-6 rounded-[2rem] border-2 border-dashed transition-all cursor-pointer flex flex-col items-center justify-center text-center ${formData.cvData ? 'bg-slate-900 border-slate-900 text-white' : 'bg-slate-50 border-slate-200 hover:border-orange-400 text-slate-400'}`}>
              ...
              <div className={`w-10 h-10 rounded-full flex items-center justify-center mb-3 ${formData.cvData ? 'bg-orange-600' : 'bg-white shadow-sm'}`}>
                <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d={formData.cvData ? "M5 13l4 4L19 7" : "M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12"} /></svg>
              </div>
             ...
             <div className="bg-white p-5 md:p-8 rounded-[2rem] border border-slate-100 shadow-sm space-y-4">
                ...
                <div className="flex gap-4">
                  ...
                  <div className="flex-1 space-y-1.5">
                    <label className="text-[9px] font-black text-slate-400 uppercase ml-1">Önceki Kurumlar</label>
                    <input type="text" className="w-full rounded-xl border border-slate-100 p-3 font-bold text-slate-800 outline-none bg-slate-50/30" value={formData.previousInstitutions} onChange={(e) => setFormData({...formData, previousInstitutions: e.target.value})} placeholder="Kurum adları..." />
                  </div>
                </div>
                <div className="p-4 bg-orange-50/50 rounded-xl border border-orange-100/50">
                   <p className="text-[10px] font-bold text-orange-800 leading-tight">
                     * {formData.branch} seçiminize göre 2. adımda branşınıza özel klinik vakalar yüklenecektir.
                   </p>
                </div>
             </div>
          </div>
          <div className="bg-white p-5 md:p-8 rounded-[2rem] border border-slate-100 shadow-sm">
            <h3 className="text-[10px] font-black text-orange-600 uppercase tracking-[0.2em] mb-5">04. Akreditasyonlar ve Eğitimler</h3>
            <div className="flex flex-wrap gap-2">
              {CERTIFICATION_LIST.map((cert) => (
                <button key={cert} type="button" onClick={() => toggleTraining(cert)} className={`px-3 py-2 rounded-xl text-[9px] font-black uppercase tracking-tight transition-all border ${formData.allTrainings.includes(cert) ? 'bg-orange-600 border-orange-600 text-white shadow-md' : 'bg-slate-50 border-slate-100 text-slate-400 hover:border-slate-300'}`}>
                  {cert}
                </button>
              ))}
              <input type="text" className="flex-1 min-w-[200px] bg-slate-100 rounded-xl px-4 py-2 text-[10px] font-bold outline-none" value={otherTrainings} onChange={(e) => setOtherTrainings(e.target.value)} placeholder="Diğer eğitimler..." />
            </div>
          </div>
        </div>
      );
        ...
        {currentQuestions.map((q: Question, idx: number) => (
          <div key={q.id} className="bg-white p-6 md:p-10 rounded-[2.5rem] border border-slate-100 shadow-sm relative overflow-hidden group">
            <div className="flex items-start gap-4 md:gap-6 mb-6">
              <div className="w-10 h-10 md:w-12 md:h-12 rounded-2xl bg-slate-900 text-white flex items-center justify-center text-sm md:text-lg font-black shrink-0">
                {idx + 1}
              </div>
              ...
              {q.type === 'radio' ? (
                q.options?.map((opt: string) => (
                  <button
                    ...
                    className={`text-left p-4 md:p-5 rounded-2xl border-2 transition-all font-bold text-sm md:text-base ${formData.answers[q.id] === opt ? 'bg-orange-600 border-orange-600 text-white shadow-xl scale-[1.01]' : 'bg-slate-50 border-slate-50 text-slate-600 hover:border-slate-200'}`}
                  >
                ...
                <textarea
                  ...
                  placeholder="Klinik muhakemenizi detaylandırın..."
                />
              )}
            </div>
          </div>
  ...
  return (
    <div className="max-w-5xl mx-auto px-2 md:px-4">
      <div className="bg-white/80 backdrop-blur-xl rounded-[3rem] shadow-2xl overflow-hidden border border-white">
        ...
        <div className="p-6 md:p-10 bg-white border-t border-slate-100 flex justify-between items-center sticky bottom-0 z-50">
          <button onClick={() => currentStep > 0 && setCurrentStep(c => c - 1)} className={`px-6 py-3 font-black text-[10px] uppercase tracking-widest transition-all rounded-xl ${currentStep === 0 ? 'opacity-0' : 'text-slate-400 hover:text-orange-600 hover:bg-orange-50'}`}>← GERİ DÖN</button>
          <button onClick={handleNext} className="px-8 md:px-12 py-4 bg-slate-900 text-white rounded-2xl font-black text-[10px] uppercase tracking-[0.2em] hover:bg-orange-600 shadow-xl transition-all active:scale-95">
            {currentStep === FORM_STEPS.length - 1 ? 'BAŞVURUYU TAMAMLA' : 'İLERLE →'}
          </button>
        </div>
      </div>
    </div>
