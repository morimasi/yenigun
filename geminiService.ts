 ...
 * gemini-3-flash-preview'ın akıllı muhakeme (thinking) yeteneğini kullanarak derin analiz yapar.
 */
export const generateCandidateAnalysis = async (candidate: Candidate, config: GlobalConfig): Promise<AIReport> => {
  ...
  const systemInstruction = `
    ...
    DİSİPLİN: Dahili muhakeme (Internal Reasoning) kullanarak adayın "idealize" bir profil çizip çizmediğini (Social Masking) anla.
    ANALİZ SÜRECİ: Cevaplardaki mikro-çelişkileri yakala, adayın kurum kültürüne (disiplin, resmiyet, etik) uygunluğunu derinlemesine tart.
  `;
  ...
  try {
    ...
    const response = await ai.models.generateContent({
      ...
      config: {
        ...
        // Akıllı versiyon için maksimum düşünme bütçesi
...
/**
 * Klinik Stres Testi Üreticisi
 * Adayın etik sınırlarını ve stres toleransını Flash-3'ün akıllı moduyla test eder.
 */
export const runStresSimulation = async (candidate: Candidate): Promise<SimulationResult> => {
  ...
  const systemInstruction = `
    ...
    - Dahili muhakeme yaparak adayı en çok zorlayacak etik ikilemi kurgula.
  `;
  ...
  const response = await ai.models.generateContent({
    ...
    config: {
      systemInstruction,
      responseMimeType: "application/json",
      // Simülasyon kalitesi için yüksek muhakeme bütçesi
      thinkingConfig: { thinkingBudget: 24576 },
      responseSchema: {
        type: Type.OBJECT,
        properties: {
