...
export interface AssessmentQuestion {
  ...
  options: {
    label: string;
    clinicalValue: number; // 0-100 arası yetkinlik puanı
    aiTag: string; // AI'nın gelişim planı üretirken kullanacağı etiket (örn: "aba_data_deficiency")
  }[];
}
...
export interface TrainingSlide { 
  ...
  subtitle?: string;
  content: string[]; 
  speakerNotes: string; // Yöneticinin okuyacağı gizli notlar
  interactiveElement?: {
    question: string;
    expectedAnswer: string;
    misconception: string;
  };
  visualPrompt: string; // Görsel önerisi
}
...
export interface PresentationConfig {
  ...
  slideCount: number;
  staffId?: string; // Eğer tekil kişiye özelse
}
