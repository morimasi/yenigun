...
export interface AssessmentBattery {
  ...
  description: string;
  icon: string;
  category: 'clinical' | 'ethics' | 'parent' | 'team';
...
export interface StaffAssessment {
  ...
  timestamp: number;
  category: string;
  scores: { technical: number; soft_skills: number; institutional_fit: number; };
  rawFeedback: string;
...
export interface AdvancedAnalyticsConfig {
  weights: { clinicalDepth: number; ethicalIntegrity: number; emotionalResilience: number; institutionalLoyalty: number; learningAgility: number; };
  penalties: { criticalEthicalViolation: number; inconsistentAnswers: number; lowExperienceDiscount: number; };
  thresholds: { minHiringScore: number; highPotentialCutoff: number; };
  aiCognition: { skepticismLevel: number; innovationBias: number; stressTestIntensity: number; };
}
