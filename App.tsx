  ...
  async getCandidates(): Promise<Candidate[]> {
    if (!this.isLocalMode) {
      try {
        ...
        console.warn("Cloud fetch failed, fallback to local.");
      }
    }
...
const App: React.FC = () => {
  ...
  const handleCandidateSubmit = async (data: any) => {
    ...
    try {
      const savedToCloud = await storage.saveCandidate(newCandidate);
      ...
      alert(savedToCloud ? "Kayıt Başarıyla Senkronize Edildi." : "Kayıt Yerel Hafızaya Alındı.");
      setView('admin');
    } catch (error) {
  ...
  return (
    <div className="min-h-screen bg-[#FDFDFD]">
      <nav className="bg-white/95 backdrop-blur-3xl border-b border-slate-100 sticky top-0 z-[60] shadow-sm">
        <div className="max-w-7xl mx-auto px-8 h-24 flex items-center justify-between">
          <div className="flex items-center space-x-5 cursor-pointer" onClick={() => setView('candidate')}>
            <div className="w-14 h-14 bg-slate-900 rounded-2xl flex items-center justify-center text-white font-black shadow-xl">
               <span className="text-xl">YG</span>
            </div>
            <div>
              ...
              <div className="flex items-center gap-2 mt-1.5">
                ...
                <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">
                  {connectionStatus === 'online' ? 'CANLI BAĞLANTI' : 'YEREL MOD'}
                </span>
              </div>
            </div>
          </div>
          ...
          <div className="flex bg-slate-100 p-1.5 rounded-2xl border border-slate-200/50">
            <button onClick={() => setView('candidate')} className={`px-8 py-3.5 rounded-xl text-[11px] font-black uppercase tracking-widest transition-all ${view === 'candidate' ? 'bg-white shadow-xl text-orange-600' : 'text-slate-500 hover:text-slate-800'}`}>Aday Formu</button>
            <button onClick={() => setView('admin')} className={`px-8 py-3.5 rounded-xl text-[11px] font-black uppercase tracking-widest transition-all ${view === 'admin' ? 'bg-white shadow-xl text-orange-600' : 'text-slate-500 hover:text-slate-800'}`}>Yönetici Paneli</button>
          </div>
        </div>
      </nav>
        ...
        {view === 'candidate' ? (
          ...
          <Dashboard 
            ...
            onUpdate={async (candidate) => {
              ...
              const newList = candidates.map(c => c.id === candidate.id ? candidate : c);
      ...
      {isProcessing && (
        <div className="fixed inset-0 bg-slate-900/95 backdrop-blur-2xl z-[200] flex items-center justify-center p-8">
          <div className="bg-white p-20 rounded-[4rem] shadow-2xl text-center max-w-xl border border-slate-100">
             ...
             <h3 className="text-4xl font-black text-slate-900 tracking-tighter">İşleniyor</h3>
             <p className="text-slate-500 mt-6 text-xl font-medium leading-relaxed italic">Veriler mühürleniyor ve akademi kriterleri analiz ediliyor...</p>
          </div>
        </div>
      )}
    </div>
